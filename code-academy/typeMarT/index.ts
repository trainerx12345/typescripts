...
const result = products.filter((product) => {
  if (product.preOrder === "true") {
    listProducts.push(
      product.name + " We'll send a message about your pre-order"
    );
  }
  if (Number(product.price) > 25) {
    shipping = 0;
    console.log(
      "We provide free shipping fee for the product " + "'" + product.name + "'"
    );
  }
  if (product.shipping.match("New York")) {
    taxPercent = 0.1;
  }

  let taxTotal = Number(product.price) * taxPercent;
  total = total = Number(product.price) + taxTotal + shipping;
  console.log(`The product \'${product.name}\' with a  price of $${total}`);
});
